...
**Asserts**, by firing the plugin's real hook scripts (`scripts/pre-tool-check.sh` and `scripts/post-tool-audit.sh`) with Codex's hook JSON on stdin against a real local AxonFlow stack in Community SaaS mode, with no mocks, stubs or recorded responses:
...
Codex runs each hook as a subprocess, passes the hook JSON on stdin (`{"tool_name": "exec_command", "tool_input": {"cmd": ...}}`, with `tool_response` added for PostToolUse), and blocks on exit code 2, with the reason on stderr. This leg runs the shipped scripts exactly that way, headless: it does not launch the Codex CLI. The stack, the tenant and every platform answer are real.
