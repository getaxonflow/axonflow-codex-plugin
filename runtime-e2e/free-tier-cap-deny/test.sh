#!/usr/bin/env bash
# free-tier-cap-deny: runtime E2E for the Codex plugin hooks.
#
# Fires the plugin's REAL hook scripts (scripts/pre-tool-check.sh and
# scripts/post-tool-audit.sh) with Codex's hook JSON on stdin against a REAL
...
echo "=== Free-tier cap deny (Codex hooks) ==="
...
REG_LABEL="free-tier-cap-deny-codex-$(date +%s)-$RANDOM"
...
# fire <hook> <tag> <cache dir> <hook JSON>: runs the hook the way Codex
...
echo ""
echo "=== Free-tier cap deny (Codex hooks): $PASS passed, $FAIL failed ==="
echo "Evidence: $EVIDENCE"
if [ "$FAIL" -gt 0 ]; then
  exit 1
fi
