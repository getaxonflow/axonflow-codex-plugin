...
- [ ] **Yes** — includes a runtime-path test under `runtime-e2e/<feature>/` that exercises this capability through Codex's plugin runtime (plugin installed via manifest path, triggered via tool/skill/hook). Importing the plugin's TypeScript classes directly does NOT count.
...
If user-facing, the wiring PR for the Codex plugin must land with this PR (or be linked and merged in the same release window). No "wire it later."
