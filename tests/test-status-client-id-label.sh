#!/usr/bin/env bash
# Locks in the v1.5.0 terminology contract for `scripts/recover.sh status`
# output: the user-facing label is `client_id`, NOT `tenant_id` (see
# CHANGELOG v1.5.0 and axonflow-enterprise#2230 Workstream C). Bridge
# note `(formerly tenant_id)` is allowed alongside the new label so
# users with muscle memory connect the two terms during v9 transition.
#
...
#
# Runs `scripts/recover.sh status` against an isolated $HOME with a
# fixture try-registration.json so it doesn't touch the user's real
# config.
...
RECOVER_SCRIPT="${PLUGIN_DIR}/scripts/recover.sh"

if [ ! -f "$RECOVER_SCRIPT" ]; then
  echo "FAIL: $RECOVER_SCRIPT missing"
  exit 1
fi
...
cat > "$REG_FILE" <<'EOF'
{
  "tenant_id": "cs_test_client_def456",
...
run_status() {
  ...
  HOME="$TMP_HOME" \
  bash "$RECOVER_SCRIPT" status 2>/dev/null
}
...
if [ $EXIT_CODE -ne 0 ]; then
  echo "FAIL: recover.sh status exited non-zero ($EXIT_CODE)"
  printf '%s\n' "$OUT"
  exit 1
fi

# Assertion 1: status output MUST surface the new label as a kv line.
if printf '%s\n' "$OUT" | grep -qE '^\s*client_id\s+cs_test_client_def456'; then
  echo "PASS: status output emits 'client_id' label"
  PASS=$((PASS + 1))
else
  echo "FAIL: status output missing 'client_id   cs_test_client_def456' line"
  printf '%s\n' "$OUT"
  FAIL=$((FAIL + 1))
fi
...
if printf '%s\n' "$OUT" | grep -qE '^\s*tenant_id\s+cs_'; then
...
if jq -e '.tenant_id == "cs_test_client_def456"' "$REG_FILE" >/dev/null; then
...
# Assertion 6: tier-line contract continues to hold (runtime-e2e
# v1-paid-tier/test.sh greps for "Free tier" and "Pro tier active" —
# we must NOT have broken that adjacent contract in the rename).
if printf '%s\n' "$OUT" | grep -qE 'tier\s+Free tier|tier\s+Pro tier active'; then
  echo "PASS: tier-line contract preserved (Free tier / Pro tier active text intact)"
  PASS=$((PASS + 1))
else
  echo "FAIL: tier-line contract broken (runtime-e2e v1-paid-tier test will fail)"
  printf '%s\n' "$OUT"
  FAIL=$((FAIL + 1))
fi
