...
## [1.9.0] - 2026-09-14: a policy answer without a decision now blocks the tool call, and the MCP session sends its credential with the Basic scheme
...
- **The MCP session now sends its credential with the `Basic` scheme, so a platform that checks credentials accepts it.** Codex sends an MCP header's variable exactly as it is set: it adds no scheme and expands nothing. The plugin mapped `Authorization` to `AXONFLOW_AUTH`, which holds the bare base64 the hooks prefix themselves, so on Community SaaS and Enterprise every MCP tool call (explain, overrides, audit search, policies) was refused with 401; plain Community admitted it. `.mcp.json` and `scripts/install-mcp-with-headers.sh` now map `Authorization` to a new variable, set as `export AXONFLOW_MCP_AUTHORIZATION="Basic $AXONFLOW_AUTH"`, and the installer prints that line when `AXONFLOW_AUTH` is set and the new variable is not. **Behaviour change:** with only `AXONFLOW_AUTH` set, the MCP session now sends no `Authorization` header instead of the bare one. Plain Community admits either; a platform that checks credentials refused the bare header and refuses the absent one until you export the new variable, and a credential without its scheme is never sent. The hooks are unchanged. Re-run the installer after upgrading.
...
- **Off by default, and absent rather than empty when off.** Leave `AXONFLOW_PEP_AUDIENCE` unset and no header is sent at all, so the plugin behaves byte for byte as 1.7.0 against every platform version. The distinction is load bearing: a header present with an empty value is malformed to the platform and would refuse the call, which an absent header does not. The per-call hooks append the flag only when non-empty, and the MCP installer does not write the key at all when there is no declaration.
- **Re-run `scripts/install-mcp-with-headers.sh` after changing `AXONFLOW_PEP_AUDIENCE`.** The declaration is written into `~/.codex/config.toml` at install time, because it is derived from the audience rather than being the audience itself and nothing in the shell would compute it for the long-lived MCP process.
- **The plugin declares no capabilities, and that is the honest answer.** A redaction obligation is discharged by substituting the platform's engine-masked text for the original, and AxonFlow does not permit a client to redact for itself. This plugin's hooks submit a statement and act on the verdict; they perform no substitution, so the plugin cannot establish that such an obligation would be discharged and does not claim it can.
...
- **References to the 5 new agent-callable MCP tools** in the README.
 The agent can answer `"what's my tenant ID?"`, `"what would I get on
 Pro?"`, and related questions directly via:
...
- **`scripts/recover.sh status` now surfaces tenant_id + upgrade URL.** Free-tier users need to find their `tenant_id` (`cs_<uuid>`) to paste into the Stripe Checkout custom field at `getaxonflow.com/pricing/`. The status output now reads `~/.config/axonflow/try-registration.json` (the auto-bootstrap registration file) and prints the tenant_id alongside endpoint + license-token state. Adds an `upgrade` line (default `https://getaxonflow.com/pricing/`, override via `AXONFLOW_UPGRADE_URL`) and copy-paste-ready upgrade instructions. Token still redacted to last 4 chars (no full bearer credential in stdout — see this release).
...
- **Runtime E2E coverage** for both surfaces:
 - the runtime test bundle — drives the pre-tool hook against a local capture server and asserts the `X-License-Token` header is sent (env, TOML, env-overrides-TOML, absence, malformed) plus the live agent middleware path when `/health` advertises `plugin_claim_license`.
 - the runtime test bundle — drives `recover.sh` against a local fake recovery agent and asserts the full request → verify → persist → status path, including replay rejection and `license_token` preservation across credential re-recovery.
