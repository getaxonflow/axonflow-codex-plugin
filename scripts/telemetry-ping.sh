...
#
# Note: DO_NOT_TRACK is intentionally not honored here. The OpenAI Codex CLI
# injects DO_NOT_TRACK=1 into every hook subprocess regardless of user intent,
# so it cannot represent a real opt-out signal in this context.
#
...
STAMP_FILE="${STAMP_DIR}/codex-plugin-telemetry-sent"
...
SDK_VERSION=$(jq -r '.version // "unknown"' "$PLUGIN_DIR/.codex-plugin/plugin.json" 2>/dev/null || echo "unknown")
...
if [ -f "$HOOKS_FILE" ]; then
  HOOK_COUNT=$(jq '[.hooks[][].hooks[]] | length' "$HOOKS_FILE" 2>/dev/null || echo "0")
fi
