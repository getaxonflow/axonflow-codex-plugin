#!/usr/bin/env bash
# Resolve the X-Axonflow-Client header value from .codex-plugin/plugin.json.
#
...
if [ -z "${AXONFLOW_CLIENT_HEADER:-}" ]; then
  ...
  if command -v jq &>/dev/null; then
    _PLUGIN_VERSION=$(jq -r '.version // empty' "${_CLIENT_HEADER_DIR}/../.codex-plugin/plugin.json" 2>/dev/null || true)
  fi
  if [ -z "$_PLUGIN_VERSION" ]; then
    _PLUGIN_VERSION="unknown"
  fi
  AXONFLOW_CLIENT_HEADER="codex-plugin/${_PLUGIN_VERSION}"
