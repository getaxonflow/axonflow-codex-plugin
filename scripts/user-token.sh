...
# axonflow-enterprise#2944 (port of the claude plugin's #2935; epic #2919:
# per-user identity + role on the fleet/MCP-server plane).
#
...
#
# FILE LOCATION (deliberate): the token file lives at the CROSS-PLUGIN
# provisioning path ~/.config/axonflow/user-token.json — the same path the
# claude plugin reads — NOT ~/.codex/axonflow.toml. Fleet provisioning
# writes ONE file per developer machine and every AxonFlow plugin on that
# machine picks up the same per-user token.
#
# Mode is 0600 inside a 0700 directory — same permissions discipline as the
# cross-plugin credential files under ~/.config/axonflow/. A file with
# non-0600 permissions is REJECTED with a stderr warning rather than loaded
# silently.
#
...
# safe to interpolate into printf-assembled JSON (no quote/backslash escaping
# needed) and into an HTTP header (no CR/LF header-splitting bytes).
...
# Refuses to read a file with non-0600 permissions.
load_user_token_from_file() {
  local file="$1"
  [ -f "$file" ] || return 1

  # Portability: try GNU stat first (CI is Linux), fall back to BSD (macOS
  # dev machines), and validate the result is numeric in both branches.
  local mode
