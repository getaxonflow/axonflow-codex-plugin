...
description: Fetch the full reasoning behind an AxonFlow policy decision - matched policies, risk level, recent hit count
---
...
Present the result as a short summary: which policy fired and the risk level. Do not suggest a session override: from v11.0.0 an override changes no verdict. If the user wants the verdict changed, explain that an administrator changes the policy in the organization's typed policy document (`create-override` has the details).
